...
{
    public class ApplicationVersionsProviderTests : AbstractAPIServiceTests
    {
        [Fact]
        public async Task ProvideVersions()
        {
            ...
            apiCacheProviderMock.Setup(cp => cp.GetOrCreate("Versions", 5, It.IsAny<Task<ApplicationVersions>>())).Returns(Task.FromResult(appVersions));
